...
fire()
{
        ...
        pos = llGetPos() + (0.1 * llRot2Left(rot));               //  Get position of avatar to create arrow
        pos = pos + vel;                //  Create arrow slightly in direction of travel
        pos.z += 0.6;                  //  Correct creation point upward to eye point 
...
default
{
     ...
     run_time_permissions(integer permissions)
    {
        ...
        if (permissions == PERMISSION_TRIGGER_ANIMATION| PERMISSION_TAKE_CONTROLS)
        {
            ...
            llStartAnimation("Left_Hand_Hold");
    ...
    listen(integer chan, string name, key id, string msg)
    {
        if(id == llGetOwner() && chan == 81)
        {
            ...
            else if(llToLower(msg) == "holster")
            {
                ...
                if (have_permissions)
                {
                    llStopAnimation("Left_Hand_Hold");
                    llReleaseControls();                    
 //                   llSetRot(<0,0,0,1>);
