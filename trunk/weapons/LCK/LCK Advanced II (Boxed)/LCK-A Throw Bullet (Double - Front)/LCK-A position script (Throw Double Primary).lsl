...
//string LCKscript = "~*LCK double-bladed saber script v1.5";
default
{
    state_entry()
    {
    ...
    listen(integer channel,string name,key id,string msg)
    {
        if(id == llGetOwner())
        {
            if(llGetSubString(llToLower(msg),0,7) == "/ls hilt")
            {
...
//                    llSetScriptState(LCKscript,TRUE);
                    llOwnerSay("It is now safe to pick the throw bullet up and put it in your saber.");
