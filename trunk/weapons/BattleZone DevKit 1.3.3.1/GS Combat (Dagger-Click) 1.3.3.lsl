...
init()
{
    ...
    if (llGetAttached() > 0) {
        ...
        llWhisper(hChannel, "sheathed dagger");
    } else {
...
default
{
    ...
    attach(key agent)
    {
        ...
        if (agent == NULL_KEY && ready) llWhisper(hChannel, "sheathed dagger");
    }
    ...
    run_time_permissions(integer permission)
    {
        if (permission > 0)
        {
            ...
            // any other kind of weapon. Suggested names are listed in the
            // documentation under Weapons System Protocol.
            llWhisper(hChannel, "drawn dagger");
        } else {
            if (hHandle) {
    ...
    control(key id, integer held, integer change) {
        ...
        if (pressed & (CONTROL_ML_LBUTTON | CONTROL_LBUTTON)) {
            ...
            llSensor("", "", AGENT, 2.0, PI/4);
        }
    }
    
    sensor(integer tnum)
    {
        llWhisper(20, "dagger," + name + "," + llDetectedName(0));
        llSleep(0.2);
