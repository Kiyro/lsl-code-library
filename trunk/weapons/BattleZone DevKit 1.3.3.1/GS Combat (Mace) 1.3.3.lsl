...
//
// This is the script you should put into a mace, or melee weapon.
...
hit(string type, string anim, string sound)
{
    ...
    // Timing is important. Based on kick, punch, and blunt attack-types, these
...
default
{
    ...
    attach(key agent)
    {
        ...
        if (agent == NULL_KEY && ready) llWhisper(hChannel, "sheathed mace");
    }
    ...
    control(key id, integer held, integer change) {
        ...
        if (down & (CONTROL_ML_LBUTTON | CONTROL_LBUTTON))
        {
            // Now depending on which direction is pressed, act accordingly.
            ...
            if (pressed  & CONTROL_FWD)                                 hit("blunt", "sword_strike_R", "");
            else if (pressed  & (CONTROL_LEFT | CONTROL_ROT_LEFT))      hit("punch", "punch_L", "");
            else if (pressed  & (CONTROL_RIGHT | CONTROL_ROT_RIGHT))    hit("kick", "kick_roundhouse_R", "");
            else if (pressed  & CONTROL_BACK)                           hit("punch", "punch_onetwo", "");
        }
    }
    ...
    sensor(integer tnum)
    {
        ...
        if (strike == "blunt")      llPlaySound("9850f8df-c86a-80d9-eca7-d6f12aae6f9e", 1);
