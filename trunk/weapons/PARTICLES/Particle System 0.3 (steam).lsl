...
float minSpeed = 0.01;            // Min speed each particle is spit out at
string texture;                 // Texture used for particles, default used if blank
float startAlpha = 0.3;           // Start alpha (transparency) value
float endAlpha = 0.1;           // End alpha (transparency) value
vector startColor = <.9,.9,.9>;    // Start color of particles <R,G,B>
vector endColor = <.9,.9,.9>;      // End color of particles <R,G,B> (if interpColor == TRUE)
vector startSize = <0.1,0.1,0.1>;     // Start size of particles 
vector endSize = <0.1,0.1,0.1>;       // End size of particles (if interpSize == TRUE)
vector push = <0,0,.2>;          // Force pushed on particles

// System paramaters
float rate = 0.10;            // How fast (rate) to emit particles
