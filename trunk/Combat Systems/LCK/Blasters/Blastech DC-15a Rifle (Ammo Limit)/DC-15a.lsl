...
integer maxammo = 150;
integer ammo = 150;
float SPEED         = 50.0;         //  Speed of arrow in meters/sec
...
//        llRezObject("bullet", pos, vel, rot, LIFETIME); 
    ...
    {
        llRezObject("DC-15a Clip", pos + <0,0,-0.5>, <0,0,1>, rot, 0);
        armed = FALSE;
        llOwnerSay("Reloading DC-15a Rifle");
        llSetTimerEvent(4);
    }
...
//        llTriggerSound("da0c3f70-fc3c-68f4-e8d6-557fb2270fdb", 1.0); //  Make the sound of the arrow being shot
        llRezObject("GGMissile", pos, vel, rot, 1); 
}
...
default
{
    ...
    attach(key attachedAgent)
    {
        ...
        {
            llOwnerSay("To Draw DC-15a, type /80 draw. To sling it on your back, type /80 sling.");
        }
    }
    ...
    control(key name, integer held, integer change) 
    {
        ...
        // 
        if ((held & CONTROL_ML_LBUTTON) && missile == 0 )
        {
            //  If left mousebutton is pressed, fire arrow 
            fire();
        }
        else if((change & held & CONTROL_ML_LBUTTON) && missile == 1)
        {
            firemissile();
            missile = 0;
        }
 ...
 //                   llSetRot(<0,0,0,1>);
            ...
            else if(llToLower(msg) == "missile")
            {
                missile = 1;
                llOwnerSay("Rocket Launcher Armed.");
            }
            else if(llToLower(msg) == "bolts")
            {
                missile = 0;
                llOwnerSay("Rocket Launcher Disarmed.");
            }
        }
    }
}
