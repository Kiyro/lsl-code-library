...
float minSpeed = 0.3;            // Min speed each particle is spit out at
string texture;                 // Texture used for particles, default used if blank
float startAlpha = 0.3;           // Start alpha (transparency) value
float endAlpha = 0.1;           // End alpha (transparency) value
vector startColor = <0,0.3,1>;    // Start color of particles <R,G,B>
vector endColor = <0,0.3,1>;      // End color of particles <R,G,B> (if interpColor == TRUE)
vector startSize = <4.1,0.1,1.1>;   // Start size of particles 
vector endSize = <4.2,0.2,1.2>;       // End size of particles (if interpSize == TRUE)
...
integer count = 90;        // How many particles to emit per BURST 
float outerAngle = 0.1;    // Outer angle for all ANGLE patterns
float innerAngle = 1;    // Inner angle for all ANGLE patterns
