          ...
          if(llToLower(msg) == "on" || llToLower(msg) == "both on")
            {
                ...
                llMessageLinked(LINK_SET,0,"2HIDE CAL",NULL_KEY);
            ...
            else if(llToLower(llGetSubString(msg,0,6)) == "color2 ")
            {
                ...
                llMessageLinked(LINK_SET,0,"2COLOR " + (string)saberColor,NULL_KEY);
            }            
        ...
        if(held & CONTROL_LBUTTON || held & CONTROL_ML_LBUTTON)
        {
            ...
            if((change & held & CONTROL_ROT_LEFT) | (change & ~held & CONTROL_LEFT))
            {
                ...
                if(BLUR == TRUE)
                {
                    ...
                    {
                        llMessageLinked(LINK_SET,0,"2OFF",NULL_KEY);
                        llMessageLinked(LINK_SET,0,"2Y_BLUR",NULL_KEY);
                    }
            ...
            if(change & held & CONTROL_FWD)
            {
                ...
                if(BLUR == TRUE)
                {
                    ...
                    {
                        llMessageLinked(LINK_SET,0,"2OFF",NULL_KEY);
                        llMessageLinked(LINK_SET,0,"2X_BLUR",NULL_KEY);
                    }
                }
                llStopAnimation(style + "_enguard");
            ...
            if(num != 0)
            {
                ...
                if (damagestatus == 2)
                {
                    for(i = 0; i < num; i++)
                    {
                        ...
                        { 
                            llShout(532254, llDetectedKey(i));
                        }
                        else if(llDetectedType(i) & SCRIPTED)
                        {
