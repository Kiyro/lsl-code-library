...
vector startColor = <0,0,0>;     // RGB color, black<0,0,0> to white<1,1,1>(*)
vector   endColor = <0.5, 0.5, 0.5>;     // 
float  startAlpha = 0.25;         // 0.0 to 1.0(*), lower = more transparent
float    endAlpha = 0.0;         // 
vector  startSize = <0.2,0.2,0>; // <0.04,0.04,0>(min) to <10,10,0>(max>, <1,1,0>(*)
vector    endSize = <0.5,0.5,0>; // (Z part of vector is discarded)  
string    texture = "d1df5743-efa9-8fab-0d2f-8c206931299b";          // Texture used for particles. Texture must be in prim's inventory.
...
float    rate = 0.05;   // Delay between bursts of new particles, 0.0 to 60, 0.1(*)
float     age = 8.0;   // How long each particle lives, 0.1 to 60, 10.0(*)
...
float  innerAngle = 0;  // "spread", for all ANGLE patterns, 0(default) to PI
float  outerAngle = TWO_PI;        // "tilt", for ANGLE patterns,  0(default) to TWO_PI, can use PI_BY_TWO or PI as well.
...
float       minSpeed = 0.01;     // 0.01 to ? Min speed each particle is spit out at, 1.0(*)
float       maxSpeed = 0.01;     // 0.01 to ? Max speed each particle is spit out at, 1.0(*)
