        ...
        if ((integer)llGetObjectName() > 0) {
            if (mode == 0) {
                ...
                if ((edge & level & CONTROL_ML_LBUTTON) == CONTROL_ML_LBUTTON) {
                    ...
                    llRezObject(bullet, llGetPos() + ((rez_pos + <0.0, -0.02 + llFrand(0.04), height_offset + llFrand(0.04)>) * rot), (llRot2Fwd(rot) * 100.0) + llGetVel(), <0.00000, 0.70711, 0.00000, 0.70711> * rot, 30 * mode);
                    llSleep(0.3);
                }
            }
        } else {
