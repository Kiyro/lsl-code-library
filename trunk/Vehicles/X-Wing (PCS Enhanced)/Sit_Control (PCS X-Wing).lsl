...
vector sittingPosition = <0.8, 0.0, -0.3>; //This is the position. X, Y, Z This is what you need to mess with.
...
init()
{
    ...
    llSitTarget(sittingPosition,<0.0, 0.00100, -0.00288, 0.70641>);
...
default
{
    ...
    changed(integer change)
    {
        if(change & CHANGED_LINK)
        {
            ...
            else if ( (av == NULL_KEY) && (agentKey != NULL_KEY) ) // Probably stand
            {
                ...
                llMessageLinked(LINK_SET, 0, "wing_land", NULL_KEY);
                if (RUNNING)
                {
                    llSetTimerEvent(0.0);
                    llMessageLinked(1, 0, "pilot", NULL_KEY);
                }
                 if ((llGetPermissions() & PERMISSION_TRIGGER_ANIMATION) == PERMISSION_TRIGGER_ANIMATION && llGetPermissionsKey() == agentKey) {
    ...
    listen(integer chan, string name, key id, string msg)
    {
        ...
        if ( msg == "start" )
        {
            ...
            llOwnerSay("Say \"pcs off\", \"dcs off\" or \"ccc off\" to turn off combat");                RUNNING = TRUE;
            HOLD = FALSE;
            llMessageLinked(LINK_SET, 0, "canopy_close", NULL_KEY);
        }
        else if ( msg == "stop" )
        ...
        else if(msg == "ccc on" || msg == "pcs on" || msg == "dcs on")
        {
            llMessageLinked(LINK_SET, 0, "wing_flight", NULL_KEY);
        }
