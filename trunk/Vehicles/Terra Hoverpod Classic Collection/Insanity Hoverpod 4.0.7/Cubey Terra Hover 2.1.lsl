...
float HOVER_HEIGHT = 4.0;       //  How high to hover while flying
...
float FOLLOW_SCALE = 100.0;      //  Extra boost to push upward for upcoming hills
...
default
{
    state_entry()
    {
       ...
       llSitTarget(<0, 0.0, 0.5>, <0,0,0,1>);                //  Set seating location. Change for different height AVs
    ...
    touch_start(integer num)
    {
        ...
        {
           llWhisper(0, "Designed by Cubey Terra. Visit Abbotts Aerodrome in Abbotts.");
    ...
    {
        if (message == "?" || message == "help" || message == "HELP")
        {
            llWhisper(0,"Cubey Terra Vehicle Help:");
            llWhisper(0,"  Up/Down arrows = thrust forward/back");
            llWhisper(0,"  Left/Right arrows = turn left/right");
            llWhisper(0,"  PageUp = jump");
            llWhisper(0,"  PageDown = brake");
            llWhisper(0,"  Say 'b' in chat to engage the emergency brake.");
    ...
    changed(integer change)
    {
       ...
       if(change & CHANGED_LINK)
       {
           ...
           else if((avatar == llGetOwner()) && (sit == FALSE))
           {
               //  
    ...
    run_time_permissions(integer perms)
    {
        if(perms & (PERMISSION_TAKE_CONTROLS))
        {
            ...
            llWhisper(0, "Say 'help' in chat for a list of controls.");
